...
include "./includes/conn.php";
    ...
    $pdf->SetFont('Arial','',10);
        ...
        $sql = "SELECT order_list.order_id, users.name, users.email, items.item_name, order_list.qty, items.price, SUM(price * qty) AS total_revenue, order_list.date
                    FROM order_list
        ...
        $result = mysqli_query($conn, $sql);
            while($row = mysqli_fetch_array($result)) {
                    ...
                    $pdf->Cell(35,6, $row[7],1,0);
                    $pdf->Cell(35,6, $row[1],1,0);
                    $pdf->Cell(35,6, $row[3],1,0);
                    $pdf->Cell(35,6, $row[5],1,0);
                    $pdf->Cell(35,6, $row[4],1,0);
                    $pdf->Cell(35,6, $row[6],1,1);
            }
