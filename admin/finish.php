...
<form action="item_list_action.php" method="post">
    <a type="submit" class="btn btn-warning finish-button disabled" href="item_list_action.php?order_id=<?php echo $row[0];?>">yes</a>
</form>
    </div>
  </div>
</div>
...
?>
    ...
    <script>
        ...
        if (buttonStates[i].hidden) {
                buttonsArray[i].classList.add("show");
            }else{
                buttonsArray[i].innerHTML = "Not Done"
                buttonsArray[i].style.background = "red"
            }
        }
